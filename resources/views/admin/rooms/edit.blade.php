    ...
    <div class="row d-flex justify-content-center my-5">
        <div class="col-sm-12 col-xl-8">
            <div class="bg-secondary rounded h-100 p-4">
                <form action="/admin/rooms-admin/{{ $room->id }}" method="post">
                    @csrf
                    @method('put')
                    <div class="mb-3">
    ...
    <div class="row d-flex justify-content-center">
        <div class="col-sm-6  bg-secondary p-5 rounded">
            <form action="/admin/rooms-admin/{{$room->id}}" method="post">
                ...
                <div class="mb-3">
                    <label for="exampleInputEmail1" class="form-label">Room Number</label>
                    <input type="number" name="room_number" value="{{$room->number_of_beds}}" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp">
                </div>
                <div class="mb-3">
                    <label for="exampleInputEmail1" class="form-label">Number of Beds</label>
                ...
                <input type="hidden" name="inn_id" value="{{$room->inn->id}}">
                <div class="mb-3">
                    <select name="status" class="form-select mb-3" aria-label="Default select example">
                        <option value="">Select Status</option>
                ...
                <div class="mb-3">
                    <select name="freebie_id" class="form-select mb-3 w-75 d-inline" aria-label="Default select example">
                        ...
                        @if (count($freebies) > 0)
                            @foreach ($freebies as $freebie)
                            <option value="{{$freebie->id}}" {{($room->freebie_id == $freebie->id) ? 'selected' : ''}}>with {{$freebie->freebie}}</option>
                            @endforeach
                        @endif
                    </select>
